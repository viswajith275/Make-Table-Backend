    ...
    for assignment in builder.assignments:
        ...
        if min_subject_daily_val is not None:
            for d in builder.days:
                ...
                builder.model.add(
                    sum(builder.shifts[(assignment.id, d, s)] for s in builder.slots)
                    >= min_subject_daily_val - slack
                )
...
def apply_subject_maximum_weekly_limit(builder: "TimeTableGenerator") -> None:
    ...
    for assignment in builder.assignments:
        max_subject_weekly_limit = getattr(
            assignment.subject, "max_classes_weekly", None
        )
        ...
        if max_subject_weekly_limit is not None:
            ...
            builder.model.add(
                ...
                <= max_subject_weekly_limit + slack
            )
...
def apply_subject_minimum_consecutive_limit(builder: "TimeTableGenerator") -> None:
    ...
    for assignment in builder.assignments:
        min_subject_consecutive_val = getattr(
            assignment.subject, "min_classes_consecutive", None
        )
        ...
        if min_subject_consecutive_val is not None:
            for d in builder.days:
                total_slots_per_day = [
                    builder.shifts[(assignment.id, d, s)] for s in builder.slots
                ]
                ...
                if min_subject_consecutive_val == 2:
                    for i, cur_slot in enumerate(total_slots_per_day):
                        ...
                        if neighbours:
                            ...
                            builder.model.add(
                                sum(neighbours) + slack >= 1
                            ).only_enforce_if(cur_slot)
                    ...
                    for s, cur_slot in enumerate(total_slots_per_day):
                        is_start = builder.model.new_bool_var(
                            f"start_{assignment.id}_{d}_{s}"
                        )  # For checking if the cur_slot is starting of consecutive sequence

                        if s > 0:
                            prev = total_slots_per_day[s - 1]

                            builder.model.add(is_start <= cur_slot)
                            builder.model.add(is_start <= prev.Not())
                            builder.model.add(is_start >= cur_slot + prev.Not() - 1)

                        else:
                            builder.model.add(is_start == cur_slot)

                        has_room_for_classes = (
                            s + needed_consecutive_class
                        ) < no_of_slotes

                        if has_room_for_classes:
                            for i in range(1, min_subject_consecutive_val):
                                ...
                                builder.model.add(
                                    total_slots_per_day[s + i] + slack >= 1
                                ).only_enforce_if(is_start)
...
def apply_subject_maximum_consecutive_limit(builder: "TimeTableGenerator") -> None:
    ...
    for assignment in builder.assignments:
        max_subeject_consecutive_limit = getattr(
            assignment.subject, "max_classes_consecutive", None
        )
        ...
        if max_subeject_consecutive_limit is not None:
            for d in builder.days:
                ...
                for i in range(
                    len(total_slots_per_day) - max_subeject_consecutive_limit + 1
                ):
                    ...
                    builder.model.add(
                        sum(
                            total_slots_per_day[
                                i : i + max_subeject_consecutive_limit + 1
                            ]
                        )
                        <= max_subeject_consecutive_limit + slack
                    )
    ...
    for class_, class_assignments in assigned_to_lab_class.items():
        ...
        for d in builder.days:
            for s in builder.slots:
                ...
                slack = builder.create_slack(
                    name="lab conflicts", error_msg=error_msg, weight=100_000_000
                )
            ...
            for a2 in hard_subjects[i + 1 :]:
                for s1 in builder.slots:
                    for s2 in builder.slots:
                        ...
                        both_scheduled = builder.model.new_bool_var(
                            f"both_{a1.id}_{a2.id}_{d}_{s1}_{s2}"
                        )
                        builder.model.add_min_equality(
                            both_scheduled,
                            [
                        ...
                        builder.silent_minimization.append(
                            both_scheduled * proximity_cost
                        )
