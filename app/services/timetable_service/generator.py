...
from app.models.enums import Hardness, WeekDayEnum
from app.schemas.generation import (
    TeacherAssignmentData,
    TimeTableCreationData,
    ViolationCreate,
)
from app.schemas.timetable_entry import TimeTableEntryCreate
from app.services.timetable_service.constraints import (
    class_,
    subject,
    teacher,
    teacher_assignment,
...
class TimeTableGenerator:
    def __init__(self, timetable_data: TimeTableCreationData) -> None:
        ...
        self.index_to_day: dict[int, WeekDayEnum] = {
            i: d for i, d in enumerate(WeekDayEnum)
        }
        self.day_to_index: dict[WeekDayEnum, int] = {
            d: i for i, d in enumerate(WeekDayEnum)
        }
    ...
    def create_all_shifts(self):
        ...
        for assignment in self.assignments:
            for d in self.days:
                for s in self.slots:
                    self.shifts[(assignment.id, d, s)] = self.model.new_bool_var(
                        f"assign_{assignment.id}_d{d}_s{s}"
                    )
