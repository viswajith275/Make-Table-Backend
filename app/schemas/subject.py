    ...
    def validation(self) -> Self:
        ...
        if self.min_classes_day is not None and self.max_classes_day is not None:
            ...
            if self.min_classes_day > self.max_classes_day:
                raise ValueError(
                    "min classes per day should not be greater than max classes per day!"
                )
        ...
        if self.min_classes_week is not None and self.max_classes_week is not None:
            ...
            if self.min_classes_week > self.max_classes_week:
                raise ValueError(
                    "min classes per week should not be greater than max classes per week!"
                )

        if (
            self.min_classes_consecutive is not None
            and self.max_classes_consecutive is not None
        ):
            ...
            if self.min_classes_consecutive > self.max_classes_consecutive:
                raise ValueError(
                    "min consecutive classes should not be greater than max consecutive classes!"
                )
