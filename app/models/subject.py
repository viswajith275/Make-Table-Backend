...
class Subject(Base):
    ...
    rgb_code: Mapped[str] = mapped_column()
    ...
    timetable: Mapped["TimeTable"] = relationship(
        "TimeTable", back_populates="subjects"
    )  # type: ignore
    lab_classes: Mapped[List["Class"]] = relationship(
        "Class",
        secondary=LabAssignment,
        passive_deletes=True,
        back_populates="assigned_labs",
    )  # type: ignore
    assignments: Mapped[List["TeacherAssignment"]] = relationship(
        "TeacherAssignment", back_populates="subject", cascade="all, delete-orphan"
    )  # type: ignore
