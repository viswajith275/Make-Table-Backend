...
class Class(Base):
    ...
    assigned_labs: Mapped[List["Subject"]] = relationship(
        "Subject",
        secondary=LabAssignment,
        passive_deletes=True,
        back_populates="lab_classes",
    )  # type: ignore
    assignments: Mapped[List["TeacherAssignment"]] = relationship(
        "TeacherAssignment", back_populates="class_", cascade="all, delete-orphan"
    )  # type: ignore
    entries: Mapped[List["TimeTableEntry"]] = relationship(
        "TimeTableEntry",
        back_populates="class_",
        cascade="all, delete-orphan",
        foreign_keys="TimeTableEntry.class_id",
    )  # type: ignore
    lab_entries: Mapped[List["TimeTableEntry"]] = relationship(
        "TimeTableEntry",
        back_populates="lab",
        cascade="all, delete-orphan",
        foreign_keys="TimeTableEntry.lab_id",
    )  # type: ignore
